import { LatticeViewer, PALETTE, stateColor } from './viewer.js';
...
function applyData(data) {
  ...
  viewer.load(data);
  if(compareViewer) compareViewer.load(data);
  $('timeline').max=data.frames.length-1;
  const end=data.frames.at(-1).time;
  $('endTime').textContent=end.toFixed(2);
  ...
  $('stageSize').textContent=axes.map(a=>a.length).join(' × ');
  const names={historical:'历史结果 · 真实数据',computed:'本次计算 · 真实数据',imported:'导入结果 · 用户提供'};
  $('stageSource').textContent=names[data.source]||'导入结果 · 用户提供';
  $('sidebarSource').textContent=data.source==='historical'?'历史结果回放':data.source==='computed'?'本次计算回放':'已导入结果';
  $('dataFootnote').textContent=(data.source==='historical'?'数据来源：SPPARKS 随包历史示例。':data.source==='computed'?'数据来源：本地 SPPARKS 本次计算。':'数据来源：用户导入文件；未验证物理来源。')+'状态颜色表示类别，不等同于独立晶粒或真实晶体取向。';
  resetParameters();
  updateClipRange();
  updateClip();
...
function setFrame(index) {
  ...
  $('stageTime').textContent=frame.time.toFixed(2);
  ...
  $('energyValue').textContent=frame.energy==null?'未提供':format(frame.energy);
  const initial=state.data.frames[0].energy;
  $('energyChange').textContent=initial>0&&frame.energy!=null?`${frame.energy<=initial?'↓':'↑'} ${Math.abs((frame.energy-initial)/initial*100).toFixed(1)}%  相比初始态`:'无可比较的初始能量';
  ...
  $('observationText').textContent=state.frame===0?'初始状态由随机种子生成。播放演化，观察同状态区域及能量如何变化。':state.frame===state.data.frames.length-1?'对照初始组织，观察同状态区域的变化。开启剖切可进一步查看内部。':'画面与曲线对应同一输出帧。暂停播放后，可旋转或剖切查看局部结构。';
  drawChart();
  updateSelection();
}
...
function drawChart() {
  ...
  const max=Math.max(...valid.map(f=>f.energy),1)*1.08;
  const start=frames[0].time,end=frames.at(-1).time;
  const x=t=>44+(t-start)/Math.max(end-start,1)*535, y=e=>112-e/max*96;
  ...
  for(let i=0;i<=2;i++) {const value=max*i/2; html+=`<path d="M44 ${y(value)}H579" stroke="#e4ebdf" stroke-width=".7" stroke-dasharray="3 4"/><text x="33" y="${y(value)+3}" text-anchor="end" fill="#9baa91" stroke="none" font-family="Consolas,monospace" font-size="9">${value>=1000?(value/1000).toFixed(0)+'k':value.toFixed(0)}</text>`;}
  ...
  svg.innerHTML=html;
  svg.setAttribute('aria-label',`总能量曲线。当前模型时间 ${f.time}，能量 ${f.energy??'未提供'}。`);
}
...
function setCompare(enabled){
  ...
  if(enabled&&!compareViewer){
    ...
    compareViewer.load(state.data);
    compareViewer.setClip(state.clip);
    // Share camera pose, including zoom and pan, for meaningful visual comparison.
    let syncing=false;
    const sync=(from,to)=>{if(syncing||!state.compare)return;syncing=true;to.camera.position.copy(from.camera.position);to.camera.quaternion.copy(from.camera.quaternion);to.camera.zoom=from.camera.zoom;to.camera.updateProjectionMatrix();to.controls.target.copy(from.controls.target);to.controls.update();syncing=false;};
...
$('snapshotButton').onclick=()=>{
  ...
  ctx.fillText(`SPPARKS / Potts | t = ${state.data.frames[state.frame].time} | ${clip}`,24,v.height+31);
  ctx.font='13px Microsoft YaHei';ctx.fillStyle='#94ada0';ctx.fillText(`${$('stageSource').textContent} · 状态配色（非独立晶粒编号）${state.compare?' · 左：初始态 / 右：当前态':''}`,24,v.height+62);
  canvas.toBlob(blob=>{if(!blob){toast('截图生成失败，请重试');return;}download(blob,`potts-t${state.data.frames[state.frame].time}.png`,'image/png');toast('已生成 PNG，已请求浏览器下载');},'image/png');
};

function showSource(){if(!state.data)return;const data=state.data;const p=data.parameters||{};modal('模型与数据来源',`<p>三维 Potts 模型将离散状态赋给格点，通过蒙特卡洛更新观察组织演化。本案例采用简单立方晶格、26 邻居与三向周期性边界。</p><h3>当前结果</h3><p>${escape(data.provenance?.description||'用户导入结果；物理来源未经验证')}<br>${format(data.ids.length)} 个格点 · ${data.frames.length} 帧 · 模型时间 ${data.frames[0].time}–${data.frames.at(-1).time}</p><p>参数：${Object.entries(p).map(([k,v])=>`${escape(k)} = ${escape(v)}`).join(' · ')||'未提供'}</p><h3>如何解读</h3><ul><li>颜色映射状态编号。空间上不连通的同状态区域不能直接算作同一晶粒。</li><li>总能量采用 SPPARKS 的 <code>diag_style energy</code>，累加格点能量；不是以焦耳计的实验能量。</li><li>时间与温度为模型量，未经标定不标注秒或开尔文。</li><li>逐帧展示真实输出；帧间不生成虚构的组织状态。暂停回放不暂停后台计算。</li></ul>${data.provenance?.dump?`<p>结构：<code>${escape(data.provenance.dump)}</code><br>能量：<code>${escape(data.provenance.log)}</code></p>`:''}<div class="dialog-actions"><button class="button primary-button" id="exportJson">导出当前完整结果 JSON</button></div>`,'MODEL & PROVENANCE');$('exportJson').onclick=()=>{download(JSON.stringify(data),`potts-${data.source}-result.json`);toast('已导出结构、时间序列与来源信息');};}
...
function showCases(){if(!state.data)return;const image=viewer.snapshot();modal('从一个物理问题开始',`<div class="case-feature"><img src="${image}" alt="当前 Potts 三维组织预览"/><div><span class="case-tag">CASE 001 / 3D POTTS</span><h3>晶粒长大</h3><p>当局部状态不断更新，三维组织会如何演化？结合结构与能量，观察这一过程。</p><span class="case-tag">8,000 SITES · 11 FRAMES</span></div></div><div class="dialog-actions"><button class="button primary-button" id="openDemo">载入原始示例，开始讲解 ${icon('arrow')}</button><button class="button" id="keepCurrent">继续当前实验</button></div><p>原始示例为随 SPPARKS 分发的历史结果。修改参数可发起本地新计算。</p>`,'CASE LIBRARY');$('openDemo').onclick=async()=>{try{applyData(await api('/api/demo'));setCompare(false);$('infoDialog').close();toast('已载入原始 Potts 示例');}catch(e){toast(e.message);}};$('keepCurrent').onclick=()=>$('infoDialog').close();}
...
$('integrationButton').onclick=()=>{modal('为新模型留好接口',`<p>把计算逻辑、模型描述和可视化结果分开。后续的铁电材料模型可以沿用实验台的交互框架。</p><h3>01 / 模型描述</h3><p><code>model.json</code> 声明参数、范围、输出量和显示能力。当前界面的参数表单已由该描述生成。</p><h3>02 / 结构与结果</h3><p>固定格点坐标与 ID 加上每帧的状态、时间和能量。当前可导入此格式的 Potts 结果 JSON；文件不执行代码。</p><h3>03 / 计算适配器</h3><p>新物理模型需要在 SPPARKS 中实现并编译，或接入独立求解器。自定义晶格还需提供位置、邻居关系和初始数值。导入结构并不自动实现新的能量函数。</p><h3>后续扩展</h3><p>可为极化矢量增加箭头与分量着色。首版渲染器仅支持固定晶格上的标量分类状态。</p><div class="dialog-actions"><button class="button" id="downloadModel">下载模型描述</button><button class="button" id="exportExample">导出当前结果作格式示例</button></div>`,'MODEL INTEGRATION');$('downloadModel').onclick=()=>download(JSON.stringify(state.model,null,2),'potts-model.json');$('exportExample').onclick=()=>{if(state.data)download(JSON.stringify(state.data),'potts-result-example.json');};};
$('importButton').onclick=()=>$('fileInput').click();
$('fileInput').onchange=async e=>{const file=e.target.files[0];if(!file)return;try{if(file.size>50*1024*1024)throw new Error('结果文件不得超过 50 MB');const data=validateResult(JSON.parse(await file.text()));data.source='imported';applyData(data);toast(`已导入 ${data.frames.length} 帧结果`);}catch(error){toast(error instanceof SyntaxError?'无法解析 JSON，请检查文件格式':error.message);}finally{e.target.value='';}};
...
async function init(){
  ...
  try{
    const [model,data]=await Promise.all([api('/model.json'),api('/api/demo')]);
    state.model=model;buildParameters();
